...
class CacheManager:
    ...
    def __init__(self, initial_cache=None):
        ...
        self.logger.debug_general("CacheManager initialized.")

    def load_from_cache(self, key):
        self.logger.debug_general(f"Loading data from cache for key: {key}")
    ...
    def is_cache_valid(self, cache_key, caching_time=None):
        self.logger.debug_general(f"Checking cache validity for key: {cache_key}")
        ...
        if cache_key in self.VIDEO_CACHE and 'last_updated' in self.VIDEO_CACHE[cache_key]:
            ...
            if (datetime.datetime.now() - last_updated).days < caching_time:
                self.logger.debug_general(f"Cache for key: {cache_key} is valid.")
                return True
